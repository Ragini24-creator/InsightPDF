...
module.exports.handleChatController = async function (req, res) {
    try {
        ...
        // Run RAG + Gemini
        const { answer, citations } = await askGemini(query);
        console.log(answer,citations)


        const botMsg = { id: uuid.v4(), role: 'bot', text: answer, citations, ts: Date.now() };
        await redis.rPush(`session:${sid}:history`, JSON.stringify(botMsg));
        await redis.expire(`session:${sid}:history`, SESSION_TTL);

        return res.status(200).json({ sessionId: sid, answer, citations });

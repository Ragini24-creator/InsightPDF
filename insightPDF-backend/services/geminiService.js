const { extractCitations, cleanAIText } = require("../utils/helperMethods");
 ...
 * 
 * @returns {object} - Answer from Gemini
 */
async function askGemini(query) {
    try {
        ...
        const context = passages
            .map(p => `Page ${p.pageNumber}: ${p.content}`)
            .join("\n\n")
            .slice(0, maxChars);
        // const context = passages
        //     .map(p => p.content)
        ...
        const finalAnswerRaw = response.response.candidates[0].content.parts[0].text;
        const finalAnswer = cleanAIText(finalAnswerRaw);

        const rawCitations = passages.map(p => ({ page: p.pageNumber, snippet: p.content.slice(0, 50) }));
        const filteredCitations = extractCitations(rawCitations);
